...
def load_model():
    ...
    model.load_state_dict(torch.load("multitask_it_transformer.pt", map_location=device), strict=False)
    model.eval()
...
def decontaminate(text):
    for kw in sorted(all_kw, key=len, reverse=True):
        text = re.compile(re.escape(kw), re.IGNORECASE).sub("[DOMAIN_KEYWORD]", text)
    return text

def classify(raw_text):
    clean   = decontaminate(raw_text.lower())
    ...
    domain  = sorted(routing_map.keys())[d_idx]
    return {
        "domain":     domain,
        ...
        "all_probs":  {sorted(routing_map.keys())[i]: round(float(p)*100,1)
